...
def main(
    ...
    user_agent_rewrite  : str   = typer.Option(None, '-uar', help="Optional User Agent rewrite dict value")
):



    logs_dir = init_logger(debug)
    cmpivot = CONSOLE(username=username, password=password, ip=ip, debug=debug, logs_dir=logs_dir, auser=auser, apassword=apassword, user_agent_rewrite=user_agent_rewrite)
    cmpivot.run()
